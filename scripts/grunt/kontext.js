...
(function (module) {
    ...
    function findPluginTags(doc) {
        var plugins = doc.getElementsByTagName('plugins');
        var ans = [];
        function findJsModule(root) {
            var jsm = root.getElementsByTagName('js_module');
            if (jsm.length > 0) {
        ...
        if (plugins) {
            for (var i = 0; i < plugins[0].childNodes.length; i += 1) {
                var node = plugins[0].childNodes[i];
                if (node.nodeType === 1) {
                    var jsm = findJsModule(node);
    ...
    function findAllThemeCss(doc) {
        var kontextNode = doc.getElementsByTagName('kontext')[0];
        var themeNode = null;
        var cssNode = null;
        var styles = [];

        var themeName = null;
        var srch = kontextNode.getElementsByTagName('name')[0];
        if (srch) {
            ...
            if (cssNode) {
                for (var i = 0; i < cssNode.childNodes.length; i += 1) {
                    if (cssNode.childNodes[i].nodeType === 1 || cssNode.childNodes[i].nodeType === 3) {
                        var cssPath = cssNode.childNodes[i].textContent.trim();
    ...
    module.exports.loadModulePathMap = function (confPath, isProduction) {
        ...
        var reactModule = isProduction ? 'vendor/react.min' : 'vendor/react.dev';
        var reactDomModule = isProduction ? 'vendor/react-dom.min' : 'vendor/react-dom.dev';
        var pluginMap = {
            'win' : 'empty:',
            'conf' : 'empty:',
