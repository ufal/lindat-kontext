...
class Kontext(Controller):
    ...
    # _post_dispatch())
    ANON_FORBIDDEN_MENU_ITEMS = (MainMenu.NEW_QUERY('history'), MainMenu.NEW_QUERY('wordlist'),
                                 MainMenu.CORPORA(
                                     'my-subcorpora'), MainMenu.CORPORA('new-subcorpus'),
    ...
    def _pre_dispatch(self, path, named_args, action_metadata=None):
        ...
        if len(path) > 0:
            ...
            if access_level and self.user_is_anonymous():
                ...
                raise auth.AuthException(_('Access forbidden. Please login.'))
        # plugins setup
    ...
    def _determine_curr_corpus(self, form, corp_list):
        ...
        if canonical_name in corp_list:  # user has "some" access to the corpus
            ...
            if auth.is_anonymous(self._session_get('user', 'id')):
                ...
                fallback = '%s%sfirst_form?corpname=%s' % (login_url, self.get_root_url(), cn)
            else:
                from plugins.abstract import auth
